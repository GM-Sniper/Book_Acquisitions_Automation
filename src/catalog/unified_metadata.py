...
def get_unified_metadata(title, authors, isbns, lccns=None):
    """
    Query Google Books, OpenLibrary, WorldCat, and LOC and return unified metadata fields.
    ...
    if isbns:
        for isbn in isbns:
            ...
            # WorldCat
            try:
                wc_data = wc_api.search_by_isbn(isbn)
                if wc_data and wc_data.get('isbn') and isbn in wc_data['isbn']:
                    found_by_isbn = True
                    break
                else:
                    wc_data = None
    ...
    if not found_by_isbn and not found_by_fallback and title:
        ...
        # WorldCat fallback
        try:
            wc_candidate = wc_api.search_by_title_author(title, authors)
            if wc_candidate:
                title_ratio = fuzzy_match(wc_candidate.get('title', ''), title)
                author_ratio = fuzzy_match(wc_candidate.get('author', ''), ', '.join(authors) if authors else '')
                if title_ratio >= 0.9 and author_ratio >= 0.9:
                    fallback_wc = wc_candidate
                    found_by_fallback = True
        except Exception:
            pass
    ...
    if found_by_isbn:
        ...
        use_wc = wc_data if wc_data else None
    elif found_by_fallback:
        use_gb = fallback_gb if fallback_gb else None
        use_ol = fallback_ol if fallback_ol else None
        use_wc = fallback_wc if 'fallback_wc' in locals() else None
    else:
        use_gb = None
        use_ol = None
        use_wc = None
    unified = {
        'TITLE': use_gb['title'] if use_gb and use_gb.get('title') else (use_ol['title'] if use_ol else (use_wc['title'] if use_wc else title)),
        'AUTHOR': use_gb['author'] if use_gb and use_gb.get('author') else (use_ol['author'] if use_ol else (use_wc['author'] if use_wc else ', '.join(authors) if authors else '')),
        'PUBLISHED': use_gb['publisher'] if use_gb and use_gb.get('publisher') else (use_ol['publisher'] if use_ol else (use_wc['publisher'] if use_wc else '')),
        'D.O Pub.': use_gb['published_date'] if use_gb and use_gb.get('published_date') else (use_ol['published_date'] if use_ol else (use_wc['published_date'] if use_wc else '')),
        'OCLC no.': use_wc['oclc_no'] if use_wc and use_wc.get('oclc_no') else (use_ol['oclc_no'] if use_ol and 'oclc_no' in use_ol else ''),
        'LC no.': lccn_str,
        'ISBN': use_gb['isbn'] if use_gb and use_gb.get('isbn') else (use_ol['isbn'] if use_ol else (use_wc['isbn'] if use_wc else '; '.join(isbns) if isbns else '')),
    }
    return unified 